...
# ── Tool schemas exposed to the master Claude call ───────────────────────────
...
class MasterAgent:
    ...
    def _dispatch_agent(self, tool_name: str, inputs: dict) -> str:
        ...
        # Inject any pending inbox messages into context
        ...
        # Enqueue → mark active → execute → complete (sequential guarantee)
        task_id = self.memory.enqueue_task({"agent": agent_name, "task": task_text})
        self.memory.get_next_task()  # pops and marks active

        try:
            result = agent.execute(task_text, exec_context)
        ...
        # Forward result to another agent if requested
    ...
    def run(self, user_request: str) -> str:
        """
        Run the master agent agentic loop for a given user request.
        ...
        while True:
            ...
            if response.stop_reason != "tool_use":
                # Unexpected stop — surface the message as-is
                return next(
                    (b.text for b in response.content if hasattr(b, "text")),
                    f"Stopped unexpectedly: {response.stop_reason}",
                )

            # Process all tool calls in this turn (master calls one at a time by design,
            # but handle batches defensively)
            tool_results = []
...
if __name__ == "__main__":
    ...
    # Optional session-level context passed as key=value args
    # e.g.: python master_agent.py location="New York" budget="$200"
    session_ctx: dict = {}
